...
var labels = []rune{'A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J'}
...
func Solve(path string) int {
	...
	for i := range input {
		_, input[i].handType = getHandType(input[i].value)
	}
...
func getHandType(hand string) (newHand string, handType HandType) {
	newHand = hand
	handType = HighCard

	for _, newLabel := range labels {
		labelsCounts := map[rune]int{}
		newHand := strings.Replace(hand, "J", string(newLabel), 5)

		for _, rune := range newHand {
			labelsCounts[rune]++
		}
