...
func Solve(path string) (total int) {
	...
	for i := 0; i < len(grid); i++ {
		for j := 0; j < len(grid[i]); {
			prev := []Coords{}
			if i == 0 {
				prev = append(prev, NewCoords(-1, j))
			} else if i == len(grid)-1 {
				prev = append(prev, NewCoords(len(grid), j))
			}

			if j == 0 {
				prev = append(prev, NewCoords(i, -1))
			} else if j == len(grid[i])-1 {
				prev = append(prev, NewCoords(i, len(grid[i])))
			}

			for _, c := range prev {
				gridCurr := grid.Copy()
				move(gridCurr, []Coords{c}, Visited{}, NewCoords(i, j))

				totalCurr := 0
				for _, row := range gridCurr {
...
func (g Grid) Copy() Grid {
	gNew := make(Grid, len(g))
	for i := range g {
		gNew[i] = make([]Tile, len(g[i]))
		copy(gNew[i], g[i])
	}
